...
case class LogOffsetMetadata(messageOffset: Long, // 消息的偏移量
                             segmentBaseOffset: Long = LogOffsetMetadata.UnknownSegBaseOffset, // 日志分段的基准偏移量
                             relativePositionInSegment: Int = LogOffsetMetadata.UnknownFilePosition) { // 消息在日志分段中的物理位置
